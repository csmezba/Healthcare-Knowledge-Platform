...
import { Search, Heart, Globe, User, Bell, ChevronDown, Sparkles } from "lucide-react";

interface HeaderProps {
  onSearchQuery?: (query: string) => void;
}

export default function Header({ onSearchQuery }: HeaderProps) {
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    ...
    if (searchValue.trim()) {
      ...
      if (pathname !== "/") {
        router.push(`/?search=${encodeURIComponent(searchValue)}#medicine-search-section`);
      } else {
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-white/80 backdrop-blur-md border-b border-slate-100 shadow-sm" id="takecare-header">
      {/* Breaking Medical News Ticker */}
      <div className="bg-blue-600 text-white text-xs py-2 overflow-hidden relative">
        <div className="max-w-[95%] md:max-w-[88%] lg:max-w-[80%] mx-auto px-4 sm:px-6 lg:px-8 flex items-center gap-3">
          ...
          <div className="animate-pulse flex-1 truncate font-medium">
            New FDA guidance issued for OTC continuous glucose monitors (CGM) usage • Study reveals 150 min of Zone 2 cardio reduces arterial plaque build-up by 22% • TakeCare advisory board releases pediatric influenza care protocol.
          </div>
      ...
      <div className="max-w-[95%] md:max-w-[88%] lg:max-w-[80%] mx-auto px-4 sm:px-6 lg:px-8 h-18 flex items-center justify-between gap-4">
        {/* Logo and Brand */}
        <Link 
          href="/"
        ...
        <nav className="hidden lg:flex items-center gap-8 font-sans font-medium text-sm text-slate-600">
          <Link 
            href="/"
            className={`hover:text-blue-600 transition-colors py-2 relative ${pathname === "/" ? "text-blue-600 border-b-2 border-blue-600 font-semibold" : ""}`}
          >
            Home & Insights
          </Link>
          ...
          <div className="relative">
            <button 
              ...
              className="hover:text-blue-600 transition-colors py-2 flex items-center gap-1 cursor-pointer"
            >
          ...
          <Link 
            href="/calculators"
            className={`hover:text-blue-600 transition-colors py-2 ${pathname === "/calculators" ? "text-blue-600 border-b-2 border-blue-600 font-semibold" : ""}`}
          >
            Health Calculators
          </Link>
          ...
          <Link 
            href="/authors"
            className={`hover:text-blue-600 transition-colors py-2 ${pathname === "/authors" ? "text-blue-600 border-b-2 border-blue-600 font-semibold" : ""}`}
          >
            Advisory Board
          </Link>
        </nav>
        ...
        <div className="flex items-center gap-3">
          ...
          <form onSubmit={handleSearchSubmit} className="relative hidden md:block">
            ...
            />
            <button type="submit" className="absolute right-3 top-2.5 text-slate-400 hover:text-blue-600 cursor-pointer">
          ...
          <Link 
            href="/ai-assistant" 
            className="flex items-center gap-1.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-xs font-semibold px-4 py-2.5 rounded-full shadow-md shadow-blue-100 hover:shadow-lg hover:from-blue-700 hover:to-indigo-700 transition-all cursor-pointer"
            id="ai-assistant-btn"
          >
          ...
          <button 
            ...
            className="p-2 text-slate-500 hover:text-slate-900 hover:bg-slate-100 rounded-lg text-xs font-bold font-mono transition-colors cursor-pointer"
          ...
          <div className="relative">
            <button 
              ...
              className="p-2 text-slate-500 hover:text-slate-900 hover:bg-slate-100 rounded-lg transition-colors relative cursor-pointer"
            >
